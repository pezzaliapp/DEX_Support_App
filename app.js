...
/* === i18n (IT/EN) — minimal live switch === */
(function(){
  const dict = {
    it: {
      nav_overview: "Overview",
      nav_sim21: "21‑cm Simulator",
  ...
  function applyLang(lang){
    ...
    $$('[data-i]').forEach(el=>{ const k=el.getAttribute('data-i'); if(d[k]) el.textContent=d[k]; });
    document.documentElement.setAttribute('lang', lang==='en'?'en':'it');
    try{ localStorage.setItem('dex_lang', lang); }catch(e){}
  }
  const btnIT=document.getElementById('btnIT'), btnEN=document.getElementById('btnEN');
  btnIT && btnIT.addEventListener('click', ()=> applyLang('it'));
  btnEN && btnEN.addEventListener('click', ()=> applyLang('en'));
  let lang='it'; try{ lang = localStorage.getItem('dex_lang') || 'it'; }catch(e){}
  applyLang(lang);
})();
