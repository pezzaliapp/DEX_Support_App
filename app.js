...
window.addEventListener('beforeinstallprompt',(e)=>{e.preventDefault();deferredPrompt=e;btnInstall&&(btnInstall.disabled=false);});
...
/* Science Parameters */
const s_freq=$('#s_freq'),s_freqVal=$('#s_freqVal'),s_elem=$('#s_elem'),s_elemVal=$('#s_elemVal'),s_theta=$('#s_theta'),s_thetaVal=$('#s_thetaVal'),s_z=$('#s_z'),s_lambda=$('#s_lambda'),s_D=$('#s_D'),s_fov=$('#s_fov'),s_age=$('#s_age'),s_T21=$('#s_T21'),s_scale=$('#s_scale');
...
function updateScience(){ const fMHz=parseFloat(s_freq.value),d=parseFloat(s_elem.value),th_arc=parseFloat(s_theta.value);
  ...
  const gAmp=-0.3,gMu=28,gSigma=4; const T21_mK = gAmp*Math.exp(-0.5*Math.pow((fMHz-gMu)/gSigma,2))*1000; s_T21.textContent=T21_mK.toFixed(1);
  const DA = DcMpc(z)/(1+z); const scale = arcminToRad(th_arc)*DA; s_scale.textContent = scale.toFixed(2);
}
